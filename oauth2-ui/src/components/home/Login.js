...
import { OAuth2Api } from '../misc/OAuth2Api'
import {getSocialLoginUrl, handleLogError} from '../misc/Helpers'
...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data, accessToken, refreshToken } = await OAuth2Api.authenticate(username, password)

      Auth.userLogin({ data, accessToken, refreshToken })
      ...
      <Grid textAlign='center'>
        <Grid.Column style={{ maxWidth: 450 }}>
          ...
          <Menu compact icon='labeled'>
            ...
            <Menu.Item name='facebook' href={getSocialLoginUrl('facebook')}>
