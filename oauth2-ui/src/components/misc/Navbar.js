...
function Navbar() {
  ...
  const adminPageStyle = () => {
    const user = getUser()
    return user && user.data.roles[0] === 'ROLE_ADMIN' ? { "display": "block" } : { "display": "none" }
  }

  const userPageStyle = () => {
    const user = getUser()
    return user && user.data.roles[0] === 'ROLE_USER' ? { "display": "block" } : { "display": "none" }
  }
  ...
  return (
    <Menu inverted color='purple' stackable size='massive' style={{borderRadius: 0}}>
      <Container>
        <Menu.Item header>OAuth2-UI</Menu.Item>
