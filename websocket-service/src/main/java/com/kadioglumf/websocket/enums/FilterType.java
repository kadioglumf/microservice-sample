...
import com.kadioglumf.websocket.payload.request.search.ConditionRequest;
import com.kadioglumf.websocket.payload.request.search.FilterRequest;
import com.kadioglumf.websocket.payload.request.search.Operator;
...
public enum FilterType {
  EQUAL {
    public <T> Predicate build(
      ...
      Object value =
          request.getFieldType().parse(validateValueAndReturn(condition.getValue()).toString());
      Expression<?> key =
          searchExpressionType.getExpression(root, request.getKey(), searchRoleType);
  ...
  BETWEEN {
    public <T> Predicate build(
      ...
      Object value =
          request.getFieldType().parse(validateValueAndReturn(condition.getValue()).toString());
      Object valueTo =
          request.getFieldType().parse(validateValueAndReturn(condition.getValueTo()).toString());
  ...
  public Object validateValueAndReturn(Object obj) {
    if (obj == null) {
      throw new BusinessException(ExceptionConstants.FILTER_PARAMETER_CANNOT_NULL_ERROR);
    }
    return obj;
  }
